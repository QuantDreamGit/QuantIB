...
void ResponseWrapper::orderStatus(int orderId, const std::string& status, Decimal filled, Decimal remaining,
	...
	if (DecimalFunctions::decimalToDouble(filled) > 0) {
		...
		LOG_TRACE_TAG(
			...
			"whyHeld={}, mktCapPrice={}", orderId, status, DecimalFunctions::decimalToString(filled),
			DecimalFunctions::decimalToString(remaining), avgFillPrice, permId, parentId, lastFillPrice, clientId,
			whyHeld, mktCapPrice);
	}
	else {
...
void ResponseWrapper::openOrderEnd() {
	...
	hub_.send<OpenOrdersTag, std::unordered_map<int, OpenOrders>*>(
		obj_.try_get<OpenOrderStoreTag, std::unordered_map<int, OpenOrders>>());
}
