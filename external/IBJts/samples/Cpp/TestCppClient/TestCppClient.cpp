...
void TestCppClient::testAlgoSamples(){
	//! [scale_order]
	Order scaleOrder = OrderSamples::RelativePeggedToPrimary("BUY", DecimalFunctions::stringToDecimal("70000"), 189, 0.01);
	AvailableAlgoParams::FillScaleParams(scaleOrder, 2000, 500, true, .02, 189.00, 3600, 2.00, true, 10, 40);
	m_pClient->placeOrder(m_orderId++, ContractSamples::USStockAtSmart(), scaleOrder);
	//! [scale_order]
	...
	m_pClient->placeOrder(m_orderId++, ContractSamples::USOptionContract(), baseOrder);
	//! [balanceimpactrisk]
