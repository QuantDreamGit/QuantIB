...
Decimal DecimalFunctions::add(Decimal decimal1, Decimal decimal2) {
    unsigned int flags;
    return Decimal(__bid64_add(decimal1, decimal2, 0, &flags));
}

Decimal DecimalFunctions::sub(Decimal decimal1, Decimal decimal2) {
    unsigned int flags;
    return Decimal(__bid64_sub(decimal1, decimal2, 0, &flags));
}

Decimal DecimalFunctions::mul(Decimal decimal1, Decimal decimal2) {
    unsigned int flags;
    return Decimal(__bid64_mul(decimal1, decimal2, 0, &flags));
}

Decimal DecimalFunctions::div(Decimal decimal1, Decimal decimal2) {
    unsigned int flags;
    return Decimal(__bid64_div(decimal1, decimal2, 0, &flags));
}
...
Decimal DecimalFunctions::doubleToDecimal(double d) {
    unsigned int flags;
    return Decimal(__binary64_to_bid64(d, 0, &flags));
}
...
Decimal DecimalFunctions::stringToDecimal(std::string str) {
    ...
    return Decimal(__bid64_from_string(const_cast<char*>(str.c_str()), 0, &flags));
}
