...
public:
	explicit ObjectHub(const std::shared_ptr<Logger>& logger)
		: logger_(logger) { LOG_DEBUG_TAG(OBJ_HUB, "Initialized correctly."); }
	...
	T& create(Args&&... args) { return create<T, T>(std::forward<Args>(args)...); }
	...
	T& insert(T obj) {
		...
		LOG_TRACE_TAG(OBJ_HUB, "Inserted object of type {} with tag {}.", NAMEOF_TYPE(T), NAMEOF_TYPE(Tag));

		return ref;
	}

	template <typename T>
	T& insert(T obj) { return insert<T, T>(std::move(obj)); }
	...
	void update_or_create(T value) {
		...
		if (auto* obj = try_get_unlocked<Tag, T>()) {
			*obj = std::move(value);
			LOG_TRACE_TAG(OBJ_HUB, "Updated object of type {} with tag {}.", NAMEOF_TYPE(T), NAMEOF_TYPE(Tag));
			return;
		}
	...
	template <typename T>
	void update_or_create(T value) { update_or_create<T, T>(std::move(value)); }

	template <typename Tag, typename T, typename... Args>
	T* get_or_create(Args&&... args) {
		std::unique_lock lock(mutex_);

		if (auto* obj = try_get_unlocked<Tag, T>()) { return obj; }

		return &create_unlocked<Tag, T>(std::forward<Args>(args)...);
	}

	template <typename T, typename... Args>
	T* get_or_create(Args&&... args) { return get_or_create<T, T>(std::forward<Args>(args)...); }
	...
	void try_append(T value) {
		...
		if (!vec) { vec = &create_unlocked<Tag, std::vector<T>>(); }

		vec->emplace_back(std::move(value));
	}

	template <typename T>
	void try_append(T value) { try_append<T, T>(std::move(value)); }
	...
	void insert_or_assign(Key key, Value value) {
		...
		if (!map) { map = &create_unlocked<Tag, std::unordered_map<Key, Value>>(); }
	...
	template <typename Tag> requires(!std::is_base_of_v<PermanentTag, Tag>)
	void erase() {
		std::unique_lock lock(mutex_);
...
private:
	...
	T* try_get_unlocked() const {
		...
		if (it->second->type() != typeid(T)) {
			LOG_WARN_TAG(OBJ_HUB, "Object with tag {} has wrong type. Expected {}, found {}.", NAMEOF_TYPE(Tag),
			             NAMEOF_TYPE(T), it->second->type().name());
			return nullptr;
		}
	...
	T& create_unlocked(Args&&... args) {
		...
		LOG_TRACE_TAG(OBJ_HUB, "Created object of type {} with tag {}.", NAMEOF_TYPE(T), NAMEOF_TYPE(Tag));
