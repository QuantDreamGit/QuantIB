...
struct ContractInfo {
	...
	ContractInfo(const int con_id_, const bool pending_details_,
	             const ContractDetails &contract_details_) : con_id(con_id_), pending_details(pending_details_),
	                                                         contract_details(contract_details_) {}
...
public:
	...
	std::optional<ContractInfo> getContractInfo(const std::string &symbol) const {
		if (const auto con_info = obj_.find_in_map<ContractSymbolStoreTag, std::string, ContractInfo>(symbol))
			return std::optional(*con_info);
		return std::nullopt;
	}
	...
	std::optional<ContractDetails> getContractDetails(const int conId) const {
		if (const auto contract = obj_.find_in_map<ContractStoreTag, int, ContractDetails>(conId))
			return
