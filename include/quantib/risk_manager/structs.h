...
struct RiskResult {
	...
	RiskResult(RiskDecision decision_,
			   std::string reason_ = "",
			   RiskAction action_ = RiskAction::NoAction,
	...
	static RiskResult pass(
		...
		RiskAction action = RiskAction::NoAction
	) {
		return {RiskDecision::Pass, reason, action};
	}

	template<typename Retry>
	static RiskResult pending(
		const std::string& reason,
		RiskAction action,
		Retry
	) {
	...
	static RiskResult pending(
		const std::string& reason,
		RiskAction action = RiskAction::Wait
	) {
		return {
			RiskDecision::Pending,
			reason,
